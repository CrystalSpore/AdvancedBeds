...
import com.snreloaded.beds.AdvancedBeds;
import com.snreloaded.beds.TexturedRenderLayersMixin;
...
public class NocturnalBedRenderer extends BlockEntityRenderer<NocturnalBedEntity> {
    private final ModelPart field_20813 = new ModelPart(64, 64, 0, 0);
    private final ModelPart field_20814;
    private final ModelPart[] legs = new ModelPart[4];

    public NocturnalBedRenderer(BlockEntityRenderDispatcher dispatcher) {
        super(dispatcher);
    ...
    public void render(NocturnalBedEntity bedBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        //SpriteIdentifier spriteIdentifier = new SpriteIdentifier(TexturedRenderLayersMixin.BED_ATLAS_TEXTURE, AdvancedBeds.id("models/block/nocturnal_bed")); //My Sprite Renderer
        SpriteIdentifier spriteIdentifier = TexturedRenderLayers.BED_TEXTURES[DyeColor.BLACK.getId()]; //Vanilla Minecraft's Sprite Identifier (modified to only use black
